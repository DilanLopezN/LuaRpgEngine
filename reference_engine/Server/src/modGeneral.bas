...
Public Sub InitServer()
    ...
    frmServer.Socket(0).LocalPort = "777"
...
Public Sub desligarServ()
    If frmServer.chkNoShutdown.Value = YES Then Exit Sub
...
Function nomeLegal(ByVal sInput As Integer) As Boolean
'a diferença é que vai aceitar ponto e traço 45,46

    If (sInput >= 45 And sInput <= 46) Or (sInput >= 65 And sInput <= 90) Or (sInput >= 97 And sInput <= 122) Or (sInput = 95) Or (sInput = 32) Or (sInput >= 48 And sInput <= 57) Then
        nomeLegal = True
    End If
