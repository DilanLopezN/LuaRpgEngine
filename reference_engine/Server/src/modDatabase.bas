...
Sub BanIndex(ByVal BanPlayerIndex As Long, ByVal BannedByIndex As Long)
    ...
    If BannedByIndex = 0 Then
        If GetPlayerAccess(BanPlayerIndex) = 1 Then
            For i = 1 To Stats.Stat_Count - 1
                SetPlayerStat BanPlayerIndex, i, 1
            Next
        
            For i = 1 To MAX_INV
                If GetPlayerInvItemNum(BanPlayerIndex, i) > 0 Then
                    If Item(GetPlayerInvItemNum(BanPlayerIndex, i)).Type = ITEM_TYPE_CURRENCY Then
                        TakeInvItem BanPlayerIndex, i, GetPlayerInvItemValue(BanPlayerIndex, i)
                    Else
                        TakeInvItem BanPlayerIndex, i, 1
                    End If
                End If
            Next
        
            SetPlayerAccess BanPlayerIndex, 0
    ...
    If BannedByIndex = 0 Then
        Print #F, IP & ",AntiHack!Login:" & Player(BanPlayerIndex).Login
    Else
        Print #F, IP & ",Banido por:" & GetPlayerName(BannedByIndex) & ".Login:" & Player(BanPlayerIndex).Login
    End If
    ...
    If BannedByIndex = 0 Then
        ...
        'Call GlobalMsg(GetPlayerName(BanPlayerIndex) & "(Gay,Filho da puta) foi banido por Hacker. Que sirva de exemplo! -'-", White)
        Call AddLog("AntiHack has banned " & GetPlayerName(BanPlayerIndex) & ".", ADMIN_LOG)

        Call ClearPlayer(BanPlayerIndex)
        SavePlayer BanPlayerIndex
        'Call Kill(App.Path & "\data\Accounts\" & Trim$(GetPlayerName(BanPlayerIndex)) & ".bin")
            
        Call AlertMsg(BanPlayerIndex, "Filho da puta!")
    Else
...
Function SecretPassOk(ByVal index As Long, ByVal Name As String) As Byte
    ' Make sure we dont try to check a name thats to small
    If Len(Trim$(Player(index).SenhaSecreta)) >= Len(Trim$(Name)) Then
        'If UCase$(Mid$(Trim$(Player(index).SenhaSecreta), 1, Len(Trim$(Name)))) = UCase$(Trim$(Name)) Then
        If UCase$(Trim$(Player(index).SenhaSecreta)) = UCase$(Trim$(Name)) Then
            SecretPassOk = YES
        End If
    End If
...
Sub AddAccount(ByVal index As Long, ByVal Name As String, ByVal Password As String, ByVal SenhaSecreta As String)
    Dim i As Long
    ...
    Player(index).SenhaSecreta = SenhaSecreta
...
Sub AddChar(ByVal index As Long, ByVal Name As String, ByVal Sex As Byte, ByVal ClassNum As Long, ByVal Sprite As Long, ByVal Vila As Byte, ByVal Elemento As Byte)
    Dim F As Long
    Dim n As Long
    Dim spritecheck As Boolean
    
    If ClassNum < 1 Or ClassNum >= SAI Then
        AlertMsg index, "Classe invalida:" & ClassNum
        Exit Sub
    End If
    ...
    If LenB(Trim$(Player(index).Name)) = 0 Then
        ...
        Player(index).Elemento(1) = Elemento
        
        Player(index).Rank = RANK_ESTUDANTE
        If Year(Now) = 2011 Then
            If Not Day(Now) > 28 Then
                Player(index).POINTS = 20
                'Player(index).VipData.DiasVIP = 1
                If Not Month(Now) > 12 Then
                    If Day(Now) < 9 Then
                        If Month(Now) < 10 Then
                            'Player(index).VipData.DataVIP = "0" & Day(Now) + 1 & "/0" & Month(Now) & "/" & Year(Now)
                        Else
                            'Player(index).VipData.DataVIP = "0" & Day(Now) + 1 & "/" & Month(Now) & "/" & Year(Now)
                        End If
                    Else
                        If Month(Now) < 9 Then
                            'Player(index).VipData.DataVIP = Day(Now) + 1 & "/0" & Month(Now) & "/" & Year(Now)
                        Else
                            'Player(index).VipData.DataVIP = Day(Now) + 1 & "/" & Month(Now) & "/" & Year(Now)
                        End If
                    End If
                    'Player(index).VipData.VIP = 2 'vip OhYeh
                End If
            End If
        End If
        ...
        Player(index).Map = START_MAP(1)
        Player(index).X = START_X(1)
        Player(index).Y = START_Y(1)
        Player(index).Dir = DIR_DOWN
...
Sub SaveMap(ByVal mapNum As Long)
    ...
    Put #F, , Map(mapNum).Weather 'temperatura
...
Sub LoadMaps()
    ...
    For i = 1 To MAX_MAPS
        ...
        Get #F, , Map(i).Name
        Get #F, , Map(i).Music
        Get #F, , Map(i).Weather 'temperatura
...
On Error Resume Next
If ClassNum > Max_Classes Then Exit Function
