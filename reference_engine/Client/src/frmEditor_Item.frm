...
If scrlArrow.value > 0 Then
    ...
    End If
    'Item(EditorIndex).Arrow = scrlArrow.Value
End Sub
...
If scrlStun.value > 0 Then
    ...
    End If
    Item(EditorIndex).StunDuration = scrlStun.value
End Sub
...
Item(EditorIndex).ExpExtra = txtEXP.text
End Sub

Private Sub txtLvlReq_Change()
If EditorIndex = 0 Or EditorIndex > MAX_ITEMS Then Exit Sub

If txtLvlReq.text = vbNullString Or Not IsNumeric(txtLvlReq.text) Then
    MsgBox "Só numeros no requerimento de level"
    txtLvlReq.text = 1
    Exit Sub
End If

If txtLvlReq.text > MAX_LEVELS Then
    MsgBox "O level requerido precisa ser no maximo" & MAX_LEVELS
    txtLvlReq.text = 1
    Exit Sub
End If

lblLevelReq.Caption = "Level req: " & txtLvlReq.text
Item(EditorIndex).LevelReq = txtLvlReq.text
End Sub
