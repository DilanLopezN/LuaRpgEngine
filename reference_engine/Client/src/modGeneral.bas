...
Public Sub Main()
    ...
    If BASS_Init(-1, 44100, 0, frmMain.hWnd, 0) = BASSFALSE Then
        Call MsgBox("Can't initialize digital sound system")
        'End
    End If
    ...
    frmMain.Caption = Options.Game_Name & "-V" & CLIENT_MAJOR & "." & CLIENT_MINOR
...
Dim i As Long
    ...
    frmMain.picStun.Picture = LoadPicture(App.Path & "\data files\graphics\stun.jpg")
    ' menu
    ...
    ' main
    frmMain.picDesafio.Picture = LoadPicture(App.Path & "\data files\graphics\gui\main\picDesafio.jpg")
    frmMain.picPergunta.Picture = LoadPicture(App.Path & "\data files\graphics\gui\main\Pergunta.jpg")
    frmMain.picTopLevel.Picture = LoadPicture(App.Path & "\data files\graphics\gui\main\toplevel.jpg")
    frmMain.picWAR.Picture = LoadPicture(App.Path & "\data files\graphics\gui\main\picwar.jpg")
    ...
    frmMain.picTrans.Picture = LoadPicture(App.Path & "\data files\graphics\gui\main\trans.jpg")
...
Public Sub MenuState(ByVal state As Long)
    ...
    Select Case state
        Case MENU_STATE_ADDCHAR
            ...
            If ConnectToServer(1) Then
                ...
                If frmMenu.optMale.value Then
                    Call SendAddChar(frmMenu.txtCName, SEX_MALE, frmMenu.cmbClass.ListIndex + 1, newCharSprite, Village, ElementoNum)
                Else
                    Call SendAddChar(frmMenu.txtCName, SEX_FEMALE, frmMenu.cmbClass.ListIndex + 1, newCharSprite, Village, ElementoNum)
                End If
            End If
        ...
        Case MENU_STATE_NEWACCOUNT
            ...
            If ConnectToServer(1) Then
                ...
                Call SendNewAccount(frmMenu.txtRUser.text, frmMenu.txtRPass.text, frmMenu.txtSenhaSecreta.text)
            End If
        ...
        Case MENU_STATE_CHANGEPASS
            ...
            If ConnectToServer(1) Then
                Call SetStatus("Connected, sending new account information...")
                SendChangePass frmMenu.txtLUser.text, frmMenu.txtLPass.text, frmMenu.txtNewPass.text, SenhaSecreta
            End If
            
            frmMenu.lblSenha2.Visible = False
            frmMenu.txtNewPass.Visible = False
            frmMenu.chkPass.Visible = True
            frmMenu.lblLAccept.Visible = True
            frmMenu.lblMudarSnormal.Visible = False
            frmMenu.lblKikarConta.Visible = False
            frmMenu.lblMudarSsecreta.Visible = False
            frmMenu.lblRecuperarSenha.Visible = False
            frmMenu.lblBlank(3).Caption = "Senha:"
            frmMenu.txtLPass.Visible = True
            
        Case MENU_STATE_KICK
            frmMenu.Visible = False
            frmMenu.picCredits.Visible = False
            frmMenu.picLogin.Visible = False
            frmMenu.picCharacter.Visible = False
            ...
            If ConnectToServer(1) Then
                Call SetStatus("Connected, sending new account information...")
                SendKikarConta frmMenu.txtLUser.text, frmMenu.txtLPass.text
            End If
    ...
    If frmLoad.Visible Then
        If Not IsConnected Then
            ...
            frmMenu.picRegister.Visible = False
            frmLoad.Visible = False
            Call MsgBox("Parece que o game tá OFFLINE,visite a comu ou o site pra ter mais info: " & GAME_WEBSITE, vbOKOnly, Options.Game_Name)
        End If
    End If
...
Dim buffer As clsBuffer, i As Long
    ...
    Set buffer = Nothing
    Call DestroyTCP
    frmMain.picAmigos.Visible = False
    ...
    frmMain.picTrans.Visible = False
...
Sub GameInit()
    ...
    Select Case Rand(0, 11)
        Case 1
            SayMsg "Tô certo! :D"
        Case 2
            SayMsg "Não tô certo! :("
        Case 3
            SayMsg "Ehissoaê brother"
        Case 4
            SayMsg "Team sasuke s2"
        Case 5
            SayMsg "Team naruto s2"
        Case 6
            SayMsg "A vida é como rodízio de churrasco: tu fica felizão enquanto come, mas no final tem que pagar a conta :/"
        Case 7
            SayMsg "Se a vida te oferece meia garrafa de óleo, 1 ovo e uma pitada de sal, faça uma maionese caseira!"
        Case 8
            SayMsg "Se meu sonho é ser pedreiro, serei o melhor pedreiro do meu bairro! O sonho é meu, pq não?!"
        Case 9
            SayMsg "O dia está bonito.. Mentira! Não sei como tá pq eu só fico aqui no meio quarto jogando essa delícia de jogo 2d!"
        Case 10
            SayMsg "oi"
        Case Else
            SayMsg "que fomeeeeeeeeeee!! Cade tu tiozinho do Lamen???"
    End Select
...
Dim i As Long
    ...
    For i = 1 To Len(sInput)
        ...
        ' Check if there's spaces
        If InStr(sInput, " ") Then
            Call MsgBox("Por favor,não use espaços", vbOKOnly, Options.Game_Name)
            Exit Function
        End If
