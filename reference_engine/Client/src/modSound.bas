...
Public Sub PlayMidiFAIL(ByVal filename As String)
...
If Song = Music_Playing Then Exit Sub
If Options.Music = 0 Then Exit Sub
If FileExist(App.Path & "\data files\music\" & Song, True) = False Then
    AddText Song & " (musica não existente)", Green
    Exit Sub
End If

    Call StopMidi

        MapSound = BASS_StreamCreateFile(BASSFALSE, StrPtr(App.Path & "\data files\Music\" & Song), 0, 0, BASS_SAMPLE_LOOP)
